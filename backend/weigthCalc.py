...
def run_dijktra(graph_struct: Graph, start: int, dest: int):
    for key in graph_struct.nodes.keys():
        graph_struct.nodes[key]["connections"] = sorted(graph_struct.nodes[key]["connections"],
                                                        key=lambda x: x["estimatedDepartureTime"])
    ...
    def weight_cost_func(u, v, d):
        ...
        if "routes" not in d:
            ...
            for route in connection_B:
                time_diff = (datetime.strptime(route["estimatedDepartureTime"],
                                               "%Y-%m-%dT%H:%M:%S") - relative_time).seconds / 60
